...
function parse(request) {
  ...
  if (typeof body === 'string') { if (Buffer.byteLength(body, 'utf8') > MAX_BODY_BYTES) fail('PAYLOAD_TOO_LARGE', 413); try { body = JSON.parse(body); } catch { fail('INVALID_JSON'); } }
  if (!body || typeof body !== 'object' || Array.isArray(body)) fail('INVALID_BODY');
  if (Buffer.byteLength(JSON.stringify(body), 'utf8') > MAX_BODY_BYTES) fail('PAYLOAD_TOO_LARGE', 413);
  return body;
}
...
function normalise(body) {
  ...
  const type = clean(resume.type, 120);
  const content = clean(resume.content, 4_200_000);
  if (!name || !content || !ALLOWED_RESUME_TYPES.has(type) || !/\.(pdf|doc|docx)$/i.test(name)) fail('INVALID_RESUME');
  application.resume = { name, type, content };
  return application;
}
...
function allowedOrigin(request) {
  ...
  try { const hostname = new URL(origin).hostname.toLowerCase(); return hostname === 'theclinixai.com' || hostname === 'www.theclinixai.com' || (process.env.VERCEL_ENV !== 'production' && hostname.endsWith('.vercel.app')); } catch { return false; }
}
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (clean(body.website, 160)) return response.status(200).json({ ok: true });
    const formAge = Date.now() - Number(body.formStartedAt);
    if (!Number.isFinite(formAge) || formAge < MIN_FORM_AGE_MS || formAge > MAX_FORM_AGE_MS) fail('INVALID_FORM_SESSION');
