(() => {
  'use strict';
  let started = false;
  const hasAnalyticsConsent = () => window.ClinixAIConsent?.analytics === true;
  ...
  startAnalytics();
  window.addEventListener('clinixai:consentchange', event => {
    if (event.detail?.analytics) startAnalytics();
  });
})();
