...
errors=[]; warnings=[]
html_files=[p for p in ROOT.glob('*.html') if p.name not in {'components.html','product-ui-gallery.html','header.html','footer.html','training.html'}]
for page in html_files:
    ...
    for link in parser.links:
        ...
        if not candidate.exists() and candidate.suffix=='': candidate=ROOT/(target+'.html')
